...
/* ------------------------------------------------------------------ */
/*  AuthInput — shadcn-styled input (local definition)                 */
/* ------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------ */
/*  Google "G" mark (inline SVG — lucide has no brand logos)           */
/* ------------------------------------------------------------------ */
const GoogleIcon = () => (
...
/*  Logo badge                                                          */
/* ------------------------------------------------------------------ */
export function LogoBadge({ letter = "S" }: { letter?: string }) {
  return (
    <motion.div
...
/* ------------------------------------------------------------------ */
/*  AuthBackground — full-screen purple gradient + noise + glow         */
/* ------------------------------------------------------------------ */
const NOISE_DATA_URI = `url("data:image/svg+xml,%3Csvg viewBox='0 0 200 200' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noiseFilter'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.65' numOctaves='3' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23noiseFilter)'/%3E%3C/svg%3E")`;

export function AuthBackground({ children }: { children: React.ReactNode }) {
  return (
    <div className="min-h-screen w-screen bg-black relative overflow-hidden flex items-center justify-center">
      {/* Purple gradient — matches the StyleMe look */}
      <div className="absolute inset-0 bg-gradient-to-b from-purple-500/40 via-purple-700/50 to-black" />

      {/* Noise texture */}
      <div
        className="absolute inset-0 opacity-[0.03] mix-blend-soft-light"
        style={{ backgroundImage: NOISE_DATA_URI, backgroundSize: "200px 200px" }}
      />
...
/* ------------------------------------------------------------------ */
/*  CardBeams — traveling light beams + corner glows                    */
/* ------------------------------------------------------------------ */
const CardBeams = () => (
...
/* ------------------------------------------------------------------ */
/*  TiltCard — 3D perspective card with glow, beams, glass shell        */
/* ------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------ */
/*  GlassField — input with leading icon + focus highlight              */
/* ------------------------------------------------------------------ */
    ...
    >
      ...
      <div className="relative flex items-center overflow-hidden rounded-lg">
        ...
        <AuthInput
          {...props}
          className={cn(
            "w-full bg-white/5 border-transparent focus:border-white/20 text-white placeholder:text-white/30 h-10 transition-all duration-300 pl-10 pr-3 focus:bg-white/10",
            trailing && "pr-10",
            className
          )}
          onFocus={(e) => {
...
/* ------------------------------------------------------------------ */
/*  SubmitButton — white CTA with loading spinner + sheen               */
/* ------------------------------------------------------------------ */
...
export function PasswordToggle({ shown, onToggle }: { shown: boolean; onToggle: () => void }) {
  return (
    <button
      ...
      aria-pressed={shown}
      className="absolute right-3 cursor-pointer rounded focus:outline-none focus-visible:ring-1 focus-visible:ring-white/40"
    >
      {shown ? (
